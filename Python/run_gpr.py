...
for i in range(output.shape[0],data.shape[0]):
    ...
    #print(muMax.compute(predictive_data=temp.data.values,model=gp))
    ...
    #params['gp_mse'] = mse.compute(edata_test,gp)
    #print(temp)
    #print(muMax.compute(predictive_data=temp,model=gp))
...
data_test = pd.read_csv(tt.get_path() + "/data/uMax/combined.csv")
timeind_test = 9
time_test = data_test.columns[timeind_test:].astype(float)
output_test = pd.DataFrame()
for i in range(output_test.shape[0],data_test.shape[0]):
    od = np.log2(data_test.iloc[i,timeind_test:].tolist())
    od = od - od[0]
    design = params = data_test.iloc[i,:timeind_test]
    params = pd.DataFrame([params.values],columns=params.index,index=[params.name])
    if design.Strain == "blank":
        continue
    key = pd.DataFrame(design).T
    key['well'] = 0
    temp = GrowthData(pd.DataFrame(list(zip(time_test.tolist(),od.tolist())),columns=['time',params.index[0]]),key)
    ...
    if output_test.shape[0]==0:
        output_test = params
    else:
        output_test = output_test.append(params)
